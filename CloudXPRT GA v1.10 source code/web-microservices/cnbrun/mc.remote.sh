...
while [ ! "$(kubectl get pods | grep "redis-service" | awk '{print $2}')" = "1/1" ]; do
    sleep 2
done
...
echo "Label control-plane node with dbtype=cassandra"
...
while [ ! "$(kubectl get pods | grep cassandra- | grep -c Running)" -ge 3 ]; do
    sleep 2
done
...
while [ ! "$(kubectl get pods | grep "db-service" | awk '{print $2}')" = "1/1" ]; do
    sleep 2
done
...
while [ ! "$(kubectl get pods | grep "^user-service" | awk '{print $2}')" = "1/1" ]; do
    sleep 2
done
...
while [ ! "$(kubectl get pods | grep "^crypt-service" | awk '{print $2}')" = "1/1" ]; do
    sleep 2
done
...
while [ ! "$(kubectl get pods | grep web-service | grep -c Running)" -ge 1 ]; do
    sleep 2
done
...
while [ ! "$(kubectl get pods | grep mc-service | grep -c Running)" -ge 1 ]; do
    sleep 2
done
...
echo "-----------------"
if [ $https = false ]; then
    curl --silent --noproxy "*" http://$masterNodeIP:31896/mc
else
    curl --silent --noproxy "*" -k https://$masterNodeIP:31443/mc
fi
echo ""
...
if [ "${@: -2:1}" = "enablehpa" ]; then
    while [ ! "$(kubectl get deploy mc-service | awk 'FNR > 1 {print $4}')" = "$min_mc_pods" ]; do
        sleep 5
    done
else
    while [ ! "$(kubectl get deploy mc-service | awk 'FNR > 1 {print $4}')" = "$max_mc_pods" ]; do
        sleep 5
    done
fi

echo "mc-service: $(kubectl get pods | grep mc-service | grep -c Running) pods are created!"
...
echo "############################"
if [ $https = false ]; then
    ./autoloader -u http://$masterNodeIP:31896/mc -c $4 -ci $5 -cl $6 -s $7 -ti $8 -e Monte
else
    ./autoloader -u https://$masterNodeIP:31443/mc -c $4 -ci $5 -cl $6 -s $7 -ti $8 -e Monte
fi
echo ""
echo "Total number of pods created during the run"
echo "-------------------------------------------"
echo "web-service pods: $(kubectl get pods | grep web-service | grep -c Running)"
echo "mc-service pods: $(kubectl get pods | grep mc-service | grep -c Running)"
