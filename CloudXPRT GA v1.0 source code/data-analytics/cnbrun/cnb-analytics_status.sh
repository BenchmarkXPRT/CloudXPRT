...
kubectl logs -n kafka cdn-webserver | grep Received | wc -l
kubectl logs -n kafka cdn-webserver | grep transaction | wc -l
kubectl logs -n kafka cdn-webserver | grep Input | wc -l
kubectl logs -n kafka cdn-webserver | grep Fail | wc -l
echo ""
...
# redirect output to file and create csv file
# ./cnb-analytics_parse-all-results.sh | sed -e 's/\s\+/,/g' > results.csv
# sed -e 's/\s\+/,/g' results.txt > results.csv
#
