...
- [Set up and run on AWS](#set-up-and-run-on-aws)
- [Set up and run on Azure](#set-up-and-run-on-azure)
- [Set up and run on GCP](#set-up-and-run-on-gcp)
...
##### Master node
1. Edit cluster_config.txt

  For each machine in the cluster, add its IPV4 address and desired hostname. One machine per line, starting with the master node.

  **Note:** Although optional, each hostname must be unique. If hostnames are not provided, Kubespray, by default will rename each host as node1, node2, ..., nodeN. This means that the master node's hostname will be changed to 'node1'.

  Example configuration for a three node cluster:

  ```
  # Do not remove the first two lines, the first node will be the master node
  # IPV4 ADDRESS      HOSTNAME(optional)
  192.168.0.1         master
  192.168.0.2         worker1
  192.168.0.3         worker2
  ```
...
2. In the master node, run "prepare-cluster.sh" script as root to perform preparation steps.
  ...
  If you do not make entries to cluster_config.txt. This script will prompt you for all the IP address for each node in your cluster. Hostnames will not be prompted for here.

  Furthermore, The script will ask if your machines are behind a proxy. If so, make sure to add the correct proxy settings for http_proxy and https_proxy when asked. This script will add those proxy settings for all the nodes to ensure that they can communicate through the Kubernetes networking plugin.
...
3. In the master node, run the "create-cluster.sh" script as root
  **Warning**: As stated in step 1, each node must have a different hostname. Kubespray, by default will rename each host as node1, node2, ..., nodeN. This means that the master node's hostname will be changed to 'node1'. If this is not desired, you may edit the 'kubespray/inventory/cnb-cluster/inventory.yaml' file with the hostnames you want by replacing all instances of 'node1' with the master's hostname. If you have multiple nodes, edit the other entries as well.
...
In your local machine, copy the results
```
scp azureuser@51.143.7.103:~/CloudXPRT-XXX/cnbrun/output/* .
```
...
In your local machine, copy the results
```
scp -i "~/.ssh/google_compute_engine" gcpuser@External_IP:~/cnb-XXX/cnbrun/output/* .
```
...
Instructions for building the benchmark from source
