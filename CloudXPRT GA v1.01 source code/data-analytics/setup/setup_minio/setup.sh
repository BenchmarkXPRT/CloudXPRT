...
while [ ! "$(kubectl get storageclass | grep "minio-data-storage" | awk '{print $2}')" = "kubernetes.io/no-provisioner" ]
do
	sleep 2;
done
...
while [ ! "$(kubectl get pv/miniodb | grep "miniodb" | awk '{print $5}')" = "Available"  ]
do
...
while [ ! "$(kubectl get pvc/minio-pv-claim | grep "minio-pv-claim" | awk '{print $2}')" = "Bound"  ]
do
...
while [ ! "$(kubectl get pods | grep "minio" | awk '{print $2}')" = "1/1" ]
do
...
while [ ! "$(kubectl get services | grep "minio-service" | awk '{print $5}')" = "9000/TCP" ]
do
