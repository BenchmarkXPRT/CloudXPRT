...
const (
	...
	mediaconfigFile = "mediaconfig.json"
	xgboostconfigFile = "cnb-analytics_config.json"

	roundComplete = `
#########################################
  CNB - %s workload completed
#########################################
`
	allDone = `
...
func main() {
	viper.SetConfigFile(xgboostconfigFile)
	viper.AddConfigPath(".")
	if err := viper.ReadInConfig(); err != nil {
		log.Fatalf("Error reading media config file, %s", err)
	}
	option := strings.ToLower(viper.GetString("runoption"))
	if option == "xgboost" {
		runXGboostCatByTitle(option)
	} else {
		fmt.Println("Configure cnb-analytics_config.json to run xgboost workload by setting the runoption to 'xgboost'")
	}
}

func main_old() {
	// or use SetConfigType("json") and SetConfigName("config")
	runArgs	:= os.Args[1:] // as media workloads need a different config file - using mediaconfig.json
	if len(runArgs) == 1{
		if runArgs[0] == "xgboost" {
			viper.SetConfigFile(xgboostconfigFile)
			viper.AddConfigPath(".")
			if err := viper.ReadInConfig(); err != nil {
				log.Fatalf("Error reading media config file, %s", err)
			...
			fmt.Println("To run xgboost workloads use command line option - xgboost")
		}
	}else {
...
func runXGboostCatByTitle(title string){
	runXGBoostWorkload(title)
}
func runXGBoostWorkload(title string) {
	titleLow := strings.ToLower(title)
	titleUpp := strings.ToUpper(title)

	var args []string
	//need to decide how to handle this //only on prem is supported
	if onCloud {
		args = []string{viper.GetString("workload." + titleLow + ".version"),
			viper.GetString("workload." + titleLow + ".cpus_per_pod"),
			viper.GetString("workload." + titleLow + ".numKAFKAmessages"),
		  viper.GetString("workload." + titleLow + ".loadgen_lambda")}
	} else {
		args = []string{viper.GetString("workload." + titleLow + ".version"),
			viper.GetString("workload." + titleLow + ".cpus_per_pod"),
			viper.GetString("workload." + titleLow + ".numKAFKAmessages"),
	...
	outputToBoth(&outputBuffer, fmt.Sprintf(roundComplete, titleUpp))
}
...
func runWorkload(title string) {
	...
	timethreshhold := viper.GetString("autoloader.timethreshold")
	if len(timethreshhold) == 0 {
		timethreshhold = "-1"
	}
	var args []string
	if onCloud {
		args = []string{viper.GetString("workload.version"),
			viper.GetString("workload.cpu"),
			viper.GetString("workload.maxpodnumber"),
		...
		args = []string{viper.GetString("workload.version"),
			viper.GetString("workload.cpu"),
			viper.GetString("workload.cpus_per_pod"),
			viper.GetString("autoloader.initialclients"),
			viper.GetString("autoloader.clientstep"),
			viper.GetString("autoloader.lastclients"),
			viper.GetString("autoloader.SLA"),
...
func writeToLog(title string, outputBuffer *bytes.Buffer) {
	err := ioutil.WriteFile(getLogfileName(title), outputBuffer.Bytes(), 0666)
	if err != nil {
...
func getLogfileName(title string) string {
...
func getReqResp(line string) (string, int, int) {
	...
	response, err := strconv.Atoi(tokens[9])
	if err != nil {
		log.Fatalf("Invalid data format found %s, error: %s", tokens[9], err.Error())
	}
	if response == 0 {
...
func postprocess(number int, title string) {
	...
	for idx, line := range allContents[0] {
		...
		if len(line) == 0 || strings.HasPrefix(line, "CLIENTS") {
			continue
		}
