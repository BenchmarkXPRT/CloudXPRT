...
# for Cloud Native Benchmark (CNB)
################################################################################
...
function validateClusterConfig {
    ...
    read -r -d ' ' -a HOSTNAMES <<< $(jq -r '.nodes | .[].hostname' $CLUSTER_CONFIG)
...
apt-get update
apt-get install python3 python3-pip openssh-server jq -y
...
source $INSTALLATION_DIRECTORY/bin/passwordless.sh
...
while read -r node; do
    ...
    ssh -n $node "mkdir /root/cnb"
    scp bin/setup-environment $node:/root/cnb/setup-environment
    ssh -n $node "apt-get install python -y"

    if [ "$SET_PROXY" = "yes" ]; then
        if [ "$REBOOT" = "yes" ]; then
            ssh -n $node "cd /root/cnb && ./setup-environment -nodetype worker -http_proxy $HTTP_PROXY -https_proxy $HTTPS_PROXY -reboot"
        else
            ssh -n $node "cd /root/cnb/ && ./setup-environment -nodetype worker -http_proxy $HTTP_PROXY -https_proxy $HTTPS_PROXY"
        fi
    else
        ssh -n $node "cd /root/cnb/ && ./setup-environment -nodetype worker -noproxy"
    fi
