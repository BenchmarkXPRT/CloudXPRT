...
if [[ ! "$(kubectl get pods --all-namespaces | grep "metrics-server" | awk '{print $3}')" = "1/1" ]]; then
	...
	kubectl create -f $CNB_DIRECTORY/metrics-server/v1.8.x.yaml
fi
...
echo "----------------------"
kubectl create -f ./cassandra/cassandra-storage.yaml
echo "Creating persistent volume for cassandra..."
sleep 3

kubectl create -f ./cassandra/cassandra-deploy.yaml
echo "Waiting for Cassandra service pod to be available..."
...
# wait for external IP address get ready
while [ -z "$(kubectl get services | grep "web-service" | awk '{print $4}' | grep "\.")" ]
do
        sleep 2;
done

web_service_ip=$(kubectl get services | grep "web-service" | awk '{print $4}')
web_service_port=$(kubectl get services | grep "web-service" | awk '{print $5}' | cut -d ":" -f 1 )
echo "Web service IP address and port:  $web_service_ip:$web_service_port"
...
then
	...
	kubectl delete storageclass cnb-storage
fi
