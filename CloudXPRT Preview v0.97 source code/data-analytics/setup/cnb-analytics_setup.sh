...
./setup.sh
sleep 30
if [ "$(docker images | awk 'FNR > 1' | awk '{print $1}' | grep xgboost | wc -l)" == '1' ]; then
...
./setup.sh
sleep 30
if [ "$(kubectl get service -n kafka | awk 'FNR > 1' | awk '{print $1}' | grep bootstrap | wc -l)" == '1' ]; then
...
./setup.sh
sleep 30
if [ "$(kubectl get service -n monitoring | awk 'FNR > 1' | awk '{print $1}' | grep *prometheus* | wc -l)" == '1' ]; then
...
./setup.sh
sleep 30
if [ "$(kubectl get pods | awk 'FNR > 1' | awk '{print $1}' | grep *minio* | wc -l)" == '1' ]; then
