...
AWS_CONFIG_FOLDER="setup_csp-gcp"

# asumption is that csp_aws contains required files with pertinent changes
# check directory exists
if [ ! ~/.ssh/google_compute_engine ]; then
  echo "the ~/.ssh/google_compute_engine file does not exist !"
  exit;
fi
...
kafka_storage_file_basename=$INSTALLATION_DIRECTORY/setup_csp-gcp/kafka-storage.yml
zookeeper_storage_file_basename=$INSTALLATION_DIRECTORY/setup_csp-gcp/zookeeper-storage.yml
...
do
  ...
  ssh -i "~/.ssh/google_compute_engine" ${node_ip} << EOF
